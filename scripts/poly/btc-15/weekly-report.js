...
/**
 * Polymarket BTC 15-min — Weekly Performance Report
 *
 * Posts a backtesting summary to #poly-btc-15-report every Monday at 09:00 UTC.
 * Covers: win rate, factor correlation, tier breakdown, best session windows.
 *
 * Usage:
 *   node scripts/poly/btc-15/weekly-report.js          → auto (Monday 09:00–09:05 UTC)
 *   node scripts/poly/btc-15/weekly-report.js --force  → run immediately
 */
...
const REPORT_HOOK  = process.env.POLY_BTC_15_REPORT_WEBHOOK;
...
function log(msg) { console.log(`[${new Date().toISOString()}] [poly-report] ${msg}`); }
...
function computeStats(trades) {
  ...
  // Factor correlation: for each factor, what's the win rate when it fired?
  ...
  for (let i = allTime.filter(t => t.signaled).length - 1; i >= 0; i--) {
    const t = allTime.filter(t2 => t2.signaled)[i];
...
function buildReport(stats) {
  ...
  const targetMsg = stats.winRate == null
    ? '*(accumulating baseline data — target: 58%+ on 100+ signals)*'
    : stats.winRate >= 0.58
      ? '✅ Above 58% target — strategy viable'
      : `⚠️ Below 58% target (${formatPct(stats.winRate)}) — threshold or factor tuning needed`;

  const lines = [
    `📊 **Poly BTC-15 Weekly Report — ${date}**`,
    SEP,
    '',
    `**THIS WEEK**`,
    ...
    '',
    SEP,
    `*Run \`!report\` in #poly-btc-15 to regenerate.*`,
...
async function main() {
  ...
  if (!REPORT_HOOK) {
    log('ERROR: POLY_BTC_15_REPORT_WEBHOOK not set in .env');
    process.exit(1);
  }
  ...
  const footer = `Poly BTC-15 • Weekly Report • ${new Date().toUTCString().slice(5, 25)} UTC`;
  await postWebhook(REPORT_HOOK, 'info', report, footer);
  log('Report posted');

  if (SIGNALS_HOOK && SIGNALS_HOOK !== REPORT_HOOK) {
    const summary = [
      `📋 **Poly BTC-15 Weekly Report Posted**`,
      `See **#poly-btc-15-report** for the full breakdown.`,
...
main().catch(e => { console.error('[poly-report] Fatal:', e.message); process.exit(1); });
