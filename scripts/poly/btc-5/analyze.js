...
/**
 * Polymarket BTC 5-min — On-Demand Analysis
 *
 * Called by the Discord bot (!analyze command) or manually.
 * Runs the full 5M→1M→1H sweep and posts the current bar's score,
 * regardless of threshold. Always posts — this is a "show me everything" read.
 *
 * Usage:
 *   node scripts/poly/btc-5/analyze.js
 *   node scripts/poly/btc-5/analyze.js --source "Manual | username"
 */
...
const MARKET_URL   = process.env.POLY_BTC_5_MARKET_URL || 'https://polymarket.com';
...
function evaluate({ price, vwap, vrvp, oiCurrent, cvd5m, ohlcv1m, ohlcv1h, utcHour }) {
  const f = {};
...
function buildEmbed(result, price, vwap, vrvp, source) {
  ...
  const now    = new Date();
  const barMin = Math.floor(now.getUTCMinutes() / 5) * 5;
  const barEnd = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), now.getUTCHours(), barMin + 5, 0));
  ...
  const cvdLine = factors.cvdDir
    ? `${factors.cvdDir === direction ? '✅' : '❌'} CVD: 1M momentum ${factors.cvdDir === 'UP' ? 'bullish' : 'bearish'} (+${factors.cvdScore})`
...
async function main() {
  if (!SIGNALS_HOOK) {
    log('ERROR: POLY_BTC_5_SIGNALS_WEBHOOK not set');
    process.exit(1);
  }

  log(`Starting on-demand analysis (source="${SOURCE}")`);

  const lock = await acquireLock(30_000, 'poly-analyze-5');
  ...
  try {
    ...
    const { vwap, oi, cvd: cvd5m } = parseStudies(studies5m);
    ...
    log(`price=$${price.toFixed(2)} vwap=${vwap?.toFixed(2)} cvd=${cvd5m?.toFixed(0)}`);

    const result = evaluate({ price, vwap, vrvp, oiCurrent: oi, cvd5m, ohlcv1m, ohlcv1h, utcHour: new Date().getUTCHours() });

    const embed  = buildEmbed(result, price, vwap, vrvp, SOURCE);
    const footer = `Poly BTC-5 • On-Demand • ${new Date().toUTCString().slice(5, 25)} UTC`;
    ...
    if (SIGNALS_HOOK) {
      await postWebhook(SIGNALS_HOOK, 'error',
        `❌ **Poly BTC-5 Analysis failed**\n**Source:** ${SOURCE}\n**Error:** ${e.message}\n**Fix:** Ensure TradingView Desktop is open on the 🕵Ace layout.`,
        'Poly BTC-5 • Analysis Error');
    }
    process.exit(1);
  } finally {
    try { if (client) await client.close(); } catch {}
    releaseLock('poly-analyze-5');
    log('Done');
  }
}
